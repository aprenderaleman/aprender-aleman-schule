...
- Wenn der Schüler technische Fragen zur App stellt, empfehle den Modus "Soporte"`
    ...
    const reply = await callAnthropicRaw('claude-3-haiku-20240307', systemPrompt, claudeMessages, 512)
    res.json({ reply })
  } catch (err) {
...
app.post('/api/chat/voice', authMiddleware, aiRateLimit, async (req, res) => {
  try {
    ...
    const reply = await callAnthropicRaw('claude-3-haiku-20240307', systemPrompt, claudeMessages, 256)
